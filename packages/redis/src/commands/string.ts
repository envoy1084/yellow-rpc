...
export const makeRedisString = (c: RedisClientType): RedisStringShape => ({
  append: effectify(c.append),
  decr: effectify(c.decr),
  decrBy: effectify(c.decrBy),
  delEx: effectify(c.delEx),
  digest: effectify(c.digest),
  get: effectify(c.get),
  getDel: effectify(c.getDel),
  getEx: effectify(c.getEx),
  getRange: effectify(c.getRange),
  getSet: effectify(c.getSet),
  incr: effectify(c.incr),
  incrBy: effectify(c.incrBy),
  incrByFloat: effectify(c.incrByFloat),
  lcs: effectify(c.lcs),
  mGet: effectify(c.mGet),
  mSet: effectify(c.mSet),
  mSetEx: effectify(c.mSetEx),
  mSetNX: effectify(c.mSetNX),
  pSetEx: effectify(c.pSetEx),
  set: effectify(c.set),
  setEx: effectify(c.setEx),
  setNX: effectify(c.setNX),
  setRange: effectify(c.setRange),
  strLen: effectify(c.strLen),
});
