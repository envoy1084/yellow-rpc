...
export const depositFundsHandler = (data: DepositFundsRequest) =>
  Effect.gen(function* () {
    ...
    // Set New Keys First
    const newUserSessionKey = data.sessionKey;
    const newEncUserSessionPrivateKey = yield* encryption.encrypt(
      data.sessionPrivateKey,
    );

    yield* appSessionRepo
      .updateAppSession(data.walletAddress, {
        userEncSessionPrivateKey: newEncUserSessionPrivateKey,
        userSessionKey: newUserSessionKey,
      })
      .pipe(
